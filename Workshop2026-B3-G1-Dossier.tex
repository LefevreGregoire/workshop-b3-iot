...
répartis dans un environnement spatial simulé. Son rôle est d'analyser
le trafic MQTT chiffré en temps réel afin d'isoler les équipements
compromis.

\textbf{Chaîne de valeur :} équipement \textgreater{} événement MQTT
\textgreater{} détection IDS \textgreater{} alerte \textgreater{}
réponse automatisée (confinement).

\textbf{Principes directeurs :} 1. \textbf{Architecture Zero Trust
...
automatisée.} L'isolement d'un équipement compromis est réalisé
instantanément par le serveur, sans intervention humaine requise. 3.
\textbf{Exécution locale (Edge).} Le traitement s'effectue localement en
mémoire (Edge Computing) pour minimiser la latence de détection.
...
\subsection{3. Objectifs (MoSCoW)}\label{objectifs-moscow}
...
\tightlist
\item
  \textbf{Flux Sécurisé :} Capteur (PIR/Digicode) \textgreater{}
  Chiffrement AES-128 Fernet + Horodatage \textgreater{} MQTT (QoS 1)
  \textgreater{} Serveur Central \textgreater{} IDS.
\item
  \textbf{Composants :}
